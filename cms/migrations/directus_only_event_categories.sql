...
INSERT INTO directus_fields (collection, field, special, interface, options, display, display_options, readonly, hidden, sort, width, translations, note, conditions, required, "group", validation, validation_message)
VALUES 
    ('Space', 'SpaceEventCategory', 'm2m', 'list-m2m', '{"template":"{{eventCategoryId.name}}","enableCreate":false,"enableSelect":true,"fields":["eventCategoryId"]}', 'related-values', '{"template":"{{eventCategoryId.name}}"}', false, false, 30, 'full', null, 'Event categories for this space', null, false, null, null, null)
ON CONFLICT (collection, field) DO NOTHING;

-- 5. EventCategory에 spaces 필드 추가 (읽기 전용)
INSERT INTO directus_fields (collection, field, special, interface, options, display, display_options, readonly, hidden, sort, width, translations, note, conditions, required, "group", validation, validation_message)
VALUES 
    ('EventCategory', 'SpaceEventCategory', 'o2m', 'list-o2m', '{"template":"{{spaceId.name}}","fields":["spaceId"]}', 'related-values', '{"template":"{{spaceId.name}}"}', true, false, 13, 'full', null, 'Spaces using this category', null, false, null, null, null)
ON CONFLICT (collection, field) DO NOTHING;
