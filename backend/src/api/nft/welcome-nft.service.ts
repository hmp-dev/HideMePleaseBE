...
export class WelcomeNftService {
	...
	async consumeWelcomeNft({
		...
		if (systemNft.contractType === "KIP-17") {
			this.logger.log(`Klaytn Mint: ${systemNft.contractType}`);
			mintRes = await this.klaytnNftService.mintToken({
		...
		}else if (systemNft.contractType === "AVAX") {
			this.logger.log(`Avalanche Mint: ${systemNft.contractType}`);
			mintRes = await this.avalancheNftService.mintToken({
